...
Editor::Editor( QObject *parent ) : QObject( parent ), scene( nullptr ) {
  ...
  mShowWires = true;
  mShowGates = true;
  //clim connect( this, &Editor::circuitHasChanged, simulationController, &SimulationController::reSortElms );
}
...
void Editor::install( Scene *s ) {
  ...
  //clim simulationController = new SimulationController( s );
  //clim simulationController->start( );
  buildSelectionRect( );
  clear( );
}
